...
/**
 * Image or document attachment data from a user message.
 * Covers both `image` blocks (screenshots) and `document` blocks (PDFs, images via document API).
 */
export interface ImageData {
  /** Unique identifier */
  id: string;
  /** MIME type — image/* for inline rendering, application/pdf for PDF indicator */
  mediaType: string;
  /** Base64 encoded data */
  data: string;
}
