...
const AIChatGroupInner = ({
  ...
  // Enhance the AI group to get display-ready data (runs in a Web Worker)
  const [enhanced, setEnhanced] = useState<EnhancedAIGroup | null>(null);
  useEffect(() => {
    let cancelled = false;
    enhanceAIGroupInWorker(aiGroup, claudeMdStats, precedingSlash).then(
      (result) => {
        if (!cancelled) setEnhanced(result);
      },
      (err) => {
        console.error('[AIChatGroup] Enhancement failed:', err);
      }
    );
    return () => {
      cancelled = true;
  ...
  const containsHighlightedError = useMemo(() => {
    if (!highlightToolUseId || !enhanced) return false;
    return containsToolUseId(enhanced.displayItems, highlightToolUseId);
  }, [enhanced, highlightToolUseId]);
  ...
  const hasToggleContent = (displayItems?.length ?? 0) > 0;
  ...
  // Show minimal skeleton while worker is computing
  if (!enhanced) {
    return (
