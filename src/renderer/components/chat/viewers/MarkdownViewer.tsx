    ...
    a: ({ href, children }) => (
      <a
        ...
        onClick={(e) => {
          e.preventDefault();
          if (href) {
            void api.openExternal(href as string);
          }
        }}
      >
        {children}
...
/**
 * Memoizes the ReactMarkdown element so re-renders with unchanged content and
 * components skip the full markdown parse + vdom diff.
 */
function useMemoizedMarkdown(content: string, components: Components): React.ReactNode {
  return useMemo(
    () => (
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- components object identity is stable (defaultComponents) or intentionally fresh (searchCtx)
    [content, components]
  );
}
...
const MarkdownViewer: React.FC<MarkdownViewerProps> = ({
  ...
  const renderedMarkdown = useMemoizedMarkdown(content, components);
