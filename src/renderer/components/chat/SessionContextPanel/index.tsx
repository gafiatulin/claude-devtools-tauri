...
export const SessionContextPanel = ({
  ...
  // Single-pass categorization, sorting, and token computation from injections
  const {
    claudeMdInjections,
    ...
    let total = 0;
    let claudeMdTok = 0;
    let mentionedFilesTok = 0;
    let toolOutputsTok = 0;
    let thinkingTextTok = 0;
    let taskCoordinationTok = 0;
    let userMessagesTok = 0;

    for (const injection of injections) {
      total += injection.estimatedTokens;
      switch (injection.category) {
        case 'claude-md':
          claudeMd.push(injection);
          claudeMdTok += injection.estimatedTokens;
          break;
        case 'mentioned-file':
          mentionedFiles.push(injection);
          mentionedFilesTok += injection.estimatedTokens;
          break;
        case 'tool-output':
          toolOutputs.push(injection);
          toolOutputsTok += injection.estimatedTokens;
          break;
        case 'thinking-text':
          thinkingText.push(injection);
          thinkingTextTok += injection.estimatedTokens;
          break;
        case 'task-coordination':
          taskCoordination.push(injection);
          taskCoordinationTok += injection.estimatedTokens;
          break;
        case 'user-message':
          userMessages.push(injection);
          userMessagesTok += injection.estimatedTokens;
          break;
      }
    }
    ...
    return {
      ...
      totalTokens: total,
      claudeMdTokens: claudeMdTok,
      mentionedFilesTokens: mentionedFilesTok,
      toolOutputsTokens: toolOutputsTok,
      thinkingTextTokens: thinkingTextTok,
      taskCoordinationTokens: taskCoordinationTok,
      userMessagesTokens: userMessagesTok,
    };
  }, [injections]);
