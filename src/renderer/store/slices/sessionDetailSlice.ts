...
/**
 * In-flight fetch deduplication: if a getSessionDetail call is already pending
 * for the same (projectId, sessionId), return the existing promise instead of
 * issuing a second IPC call.  Cleaned up in .finally() so the next navigation
 * after the current one resolves always performs a fresh fetch.
 */
const inFlightDetailFetches = new Map<string, Promise<SessionDetail | null>>();

function deduplicatedGetSessionDetail(
  projectId: string,
  sessionId: string
): Promise<SessionDetail | null> {
  const key = `${projectId}:${sessionId}`;
  const existing = inFlightDetailFetches.get(key);
  if (existing) return existing;

  const promise = api.getSessionDetail(projectId, sessionId).finally(() => {
...
export const createSessionDetailSlice: StateCreator<AppState, [], [], SessionDetailSlice> = (
  ...
  fetchSessionDetail: async (projectId: string, sessionId: string, tabId?: string) => {
    ...
    try {
      const detail = await deduplicatedGetSessionDetail(projectId, sessionId);
      ...
      if (conversation?.items) {
        ...
        // Process Visible Context with all token data — runs in a Web Worker to avoid
        // blocking the main thread. Falls back to main-thread if the worker is unavailable.
        // Pass validated directory token data so contextTracker can filter non-existent files
        const phaseResult = await computeContextInWorker(
          conversation.items,
          projectRoot,
          claudeMdTokenData,
